...
def random_color_gen(batch_size=100, category_type='both'):
    ...
    while True:
        ...
        while True:
            ...
            y = labels[i]
            x = svrt_1_img(category=y, regular=False, color_a=color_bgr, sides=None, thickness=1)
            coors, rel_pos, bad_sample = get_shapes_info(x, scrambled_negative=False, draw_b_rects=False)
            if bad_sample:
                pass
            else:
...
def random_two_chanels_gen(batch_size=64, category_type='both'):
    ...
    while True:
        ...
        while True:
            ...
            color_bgr = (int(color[2]), int(color[1]), int(color[0]))
            y = labels[i]
            x1, x2 = svrt_1_img(category=y, regular=False, color_a=color_bgr, thickness=1, separated_chanels=True)
            inputs1.append(x1)
            inputs2.append(x2)
            sd_labels.append(y)
            i += 1
