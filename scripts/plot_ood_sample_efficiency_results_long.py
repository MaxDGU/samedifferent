#!/usr/bin/env python3
"""
Plot OOD Sample Efficiency Results (Long Version)

This script loads and plots the out-of-distribution (OOD) sample efficiency 
comparison results from the LONG holdout experiment with extended adaptation steps,
comparing how FOMAML, Second-Order MAML, and Vanilla SGD perform on novel tasks.

Long version uses: 10 adaptation steps (training) / 15 adaptation steps (testing)
"""
...
def plot_ood_sample_efficiency(results, save_path=None, show_plot=True):
    ...
    title = f'Out-of-Distribution Sample Efficiency (Long Version)\nHoldout Task: "{holdout_task}" | Training Tasks: {len(train_tasks)} others\n10 Train Steps / 15 Test Steps'
