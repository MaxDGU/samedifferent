#!/usr/bin/env python3
"""
Parse Vanilla SGD Results from SLURM Output Files

This script parses SLURM output files to extract Vanilla SGD validation data points
...
def parse_sgd_from_file(filepath):
    """Parse Vanilla SGD validation data from a single SLURM output file."""
    data_points = []
    val_accuracies = []
    ...
    try:
        ...
        # Find all validation blocks for Vanilla SGD
        validation_pattern = r'--- Vanilla SGD Validation at Batch (\d+) ---\s+Data points seen: ([\d,]+)\s+Current train accuracy: [\d.]+%\s+Validation accuracy: ([\d.]+)%'
...
def plot_sgd_efficiency(all_data_points, all_accuracies, output_dir):
    """Create a sample efficiency plot for Vanilla SGD."""
    ...
    plt.title('Vanilla SGD Sample Efficiency: Data Points vs Validation Accuracy', fontsize=14, fontweight='bold')
    ...
    # Save plot
    output_path = os.path.join(output_dir, 'sgd_sample_efficiency.png')
...
def main():
    parser = argparse.ArgumentParser(description='Parse Vanilla SGD results from SLURM output files')
    parser.add_argument('--input_dir', type=str, 
                       default='/scratch/gpfs/mg7411/samedifferent/results/sample_efficiency_sgd_only/slurm_out/',
                       help='Directory containing SLURM output files')
    parser.add_argument('--output_dir', type=str, default='results/sgd_analysis',
    ...
    print("🔍 VANILLA SGD SAMPLE EFFICIENCY ANALYSIS")
    print("="*50)
    ...
    for filepath in slurm_files:
        data_points, val_accuracies = parse_sgd_from_file(filepath)
        if data_points and val_accuracies:
            all_data_points.append(data_points)
            all_accuracies.append(val_accuracies)
    
    if not all_data_points:
        print("❌ No Vanilla SGD data found in any files!")
        return
    
    print(f"\n✅ Successfully parsed {len(all_data_points)} seeds")
    
    # Create plot
    plot_sgd_efficiency(all_data_points, all_accuracies, args.output_dir)
