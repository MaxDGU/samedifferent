#!/usr/bin/env python3
"""
Multi-Seed OOD Sample Efficiency Comparison with Statistical Analysis

This script runs the holdout experiment across multiple seeds and performs
...
class MultiSeedOODAnalysis:
    """Handle multi-seed OOD experiment execution and statistical analysis."""
    ...
    def generate_slurm_script(self):
        ...
        script_path = os.path.join(self.base_args.save_dir, f'holdout_{self.base_args.holdout_task}', 'run_ood_holdout_lines.slurm')
        
        # Use the holdout comparison script
        experiment_script = 'scripts/sample_efficiency_comparison_holdout.py'
        ...
        job_name = f'ood_lines_speedrun' if hasattr(self.base_args, 'speed_run') and self.base_args.speed_run else 'ood_lines_holdout'

        slurm_script_content = f"""#!/bin/bash
#SBATCH --job-name={job_name}
#SBATCH --output={self.base_args.save_dir}/holdout_{self.base_args.holdout_task}/slurm_out/slurm_%A_%a.out
#SBATCH --error={self.base_args.save_dir}/holdout_{self.base_args.holdout_task}/slurm_out/slurm_%A_%a.err
...
echo "Running OOD Lines Holdout for seed $SEED"

python {experiment_script} \\
    --epochs {self.base_args.epochs} \\
    --seed $SEED \\
    --holdout_task {self.base_args.holdout_task} \\
...
"""
        os.makedirs(os.path.join(self.base_args.save_dir, f'holdout_{self.base_args.holdout_task}', 'slurm_out'), exist_ok=True)
        with open(script_path, 'w') as f:
            f.write(slurm_script_content)
            
        print("\n✅ Generated SLURM script for OOD holdout experiment.")
    ...
    def run_single_seed_experiment(self, seed):
        """Run the holdout experiment for a single seed."""
    ...
    def load_results(self):
        ...
        for seed in self.seeds:
            seed_dir = os.path.join(self.base_args.save_dir, f'holdout_{self.base_args.holdout_task}', f'seed_{seed}')
            ...
            for method in self.methods:
                if method == 'fomaml':
                    filename = f'fomaml_holdout_{self.base_args.holdout_task}_results.json'
                elif method == 'second_order':
                    filename = f'second_order_maml_holdout_{self.base_args.holdout_task}_results.json'
                elif method == 'vanilla':
                    filename = f'vanilla_sgd_holdout_{self.base_args.holdout_task}_results.json'
                else:
                    continue
    ...
    def create_publication_plots(self, save_dir):
        ...
        for i, point_key in enumerate(self.statistical_results['key_points']):
            ...
            if significant_comparisons:
                ...
                ax.annotate('*', xy=(x_pos, max_y + 1), fontsize=16, 
                           ha='center', va='bottom', color='red', weight='bold')
        
        # Formatting
        ax.set_xlabel('Number of Data Points Seen (millions)', fontsize=14)
        ax.set_ylabel('Out-of-Distribution Validation Accuracy (%)', fontsize=14)
        ax.set_title(f'OOD Sample Efficiency: Holdout Task "{self.base_args.holdout_task}"\n'
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, 'ood_multiseed_comparison.png'), dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(save_dir, 'ood_multiseed_comparison.pdf'), bbox_inches='tight')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, 'ood_statistical_summary.png'), dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(save_dir, 'ood_statistical_summary.pdf'), bbox_inches='tight')
    ...
    def save_statistical_report(self, save_dir):
        ...
        if final_performances:
            ...
            recommendations.append(f"Best overall OOD performance: {best_method} ({final_performances[best_method]:.1f}%)")
            recommendations.append(f"Worst overall OOD performance: {worst_method} ({final_performances[worst_method]:.1f}%)")
        ...
        with open(os.path.join(save_dir, 'statistical_summary.txt'), 'w') as f:
            f.write("OOD SAMPLE EFFICIENCY STATISTICAL ANALYSIS SUMMARY\n")
            f.write("=" * 60 + "\n\n")
            
            f.write(f"Experiment: {self.base_args.holdout_task} holdout task\n")
...
def main():
    parser = argparse.ArgumentParser(description='Multi-Seed OOD Sample Efficiency Analysis')
    
    # Experiment parameters
    parser.add_argument('--seeds', nargs='+', type=int, default=[42, 43, 44, 45, 46],
                       help='Seeds to run (default: 42 43 44 45 46)')
    parser.add_argument('--holdout_task', type=str, default='lines',
                       help='Holdout task for OOD evaluation')
    ...
    parser.add_argument('--vanilla_batch_size', type=int, default=128,
                       help='Vanilla SGD batch size')
    parser.add_argument('--inner_lr', type=float, default=0.02,
                       help='Inner loop learning rate')
    parser.add_argument('--outer_lr', type=float, default=0.001,
                       help='Outer loop learning rate')
    parser.add_argument('--vanilla_lr', type=float, default=0.0002,
    ...
    parser.add_argument('--data_dir', type=str, default='data/meta_h5/pb',
                       help='Data directory')
    parser.add_argument('--save_dir', type=str, default='results/sample_efficiency_ood_multiseed',
    ...
    parser.add_argument("--speed_run", action="store_true",
                       help="Enable speed run mode for faster execution")
    ...
    if args.speed_run:
        print('🚀 SPEED RUN MODE ENABLED for OOD Lines Holdout')
        print('Optimizations: Fewer epochs, larger batches, faster validation')
        args.epochs = 15  # Fewer epochs for speed
        args.meta_batch_size = 64  # Much larger meta batches
        args.vanilla_batch_size = 256  # Much larger vanilla batches
        args.adaptation_steps = 1  # Minimal adaptation steps
        args.val_frequency = 100  # More frequent validation
        args.inner_lr = 0.02  # Slightly higher inner LR
        args.outer_lr = 0.002  # Higher outer LR
        args.vanilla_lr = 0.0005  # Higher vanilla LR
        print(f'Speed run params: epochs={args.epochs}, meta_batch={args.meta_batch_size}, val_freq={args.val_frequency}')


    
    # Create analysis object
    analyzer = MultiSeedOODAnalysis(args, args.seeds, args.methods)
    
    # Create output directory
    output_dir = os.path.join(args.save_dir, f'holdout_{args.holdout_task}', 'multiseed_analysis')
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"🎯 MULTI-SEED OOD SAMPLE EFFICIENCY ANALYSIS")
    print(f"={'='*60}")
    print(f"Holdout task: {args.holdout_task}")
    print(f"Seeds: {args.seeds}")
    print(f"Methods: {args.methods}")
    print(f"Output directory: {output_dir}")
    print(f"{'='*60}\n")
    
    # Check if we should analyze or generate experiments
    if args.run_experiments:
        ...
        for seed in args.seeds:
            ...
            if args.skip_existing:
                seed_dir = os.path.join(args.save_dir, f'holdout_{args.holdout_task}', f'seed_{seed}')
    ...
    print("\n🚀 PHASE 1: Generating SLURM script...")
    ...
    print(f"Key files:")
    print(f"  • ood_multiseed_comparison.png - Main comparison plot")
    print(f"  • ood_statistical_summary.png - Statistical summary")
